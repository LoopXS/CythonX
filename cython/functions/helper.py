...
def gen_chlog(repo, diff):
    ...
    ch_log = tldr_log = ""
    ch = f"<b>CɪᴘʜᴇʀX ᴇxᴄlusivᴇ ʙᴏᴛ {ultroid_version} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
    ch_tl = f"CɪᴘʜᴇʀX ᴇxᴄlusivᴇ ʙᴏᴛ {ultroid_version} updates for {ac_br}:"
    d_form = "%d/%m/%y || %H:%M"
...
async def shutdown(ult, dynotype="megatron"):
    from .. import LOGS
